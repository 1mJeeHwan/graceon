...
| 감점 | 항목 | 건수 | 점수 |
|---|---|---|---|
| -10/관리자페이지 누락 | church/album/worship/sms/store 관리화면 | 5 | -50 |
| -10/기능 누락 | 카탈로그 확장 미반영(신규 카드들) | 1 | -10 |
| -5/이미지 누락 | 카탈로그 대표이미지·일부 도메인 썸네일 | ~2 | -10 |
| -10/권한 오류 | 없음 | 0 | 0 |
| -15/런타임 오류 | 없음(빌드/E2E 그린) | 0 | 0 |
| -20/배포 오류 | 없음(이미지빌드·compose 검증) | 0 | 0 |
| -10/API 누락 | 없음(전 도메인 API 존재) | 0 | 0 |
| **현재 점수** | | | **≈ 70/100** |

> 핵심: **API·운영사이트·백엔드는 만점급**. 감점은 거의 전부 **관리자 관리화면 부재 + 카탈로그 미반영**.
