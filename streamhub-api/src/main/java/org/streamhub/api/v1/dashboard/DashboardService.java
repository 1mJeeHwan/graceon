...
public class DashboardService {
    ...
    /**
     * Builds the 6-KPI strip. Cached under {@code dashboardSummary::all}; recomputed on
     * miss / after TTL. "Today" vs "yesterday" windows drive the ▲▼ comparison for the
     * revenue KPI; structural counts (open orders, low stock, active subscribers) have
     * no natural previous-period so their delta is 0.
     *
     * @return the populated summary
     */
    @Cacheable(cacheNames = "dashboardSummary", key = "'all'")
    public DashboardSummaryResponse getSummary() {
        ...
        long revenueToday = dashboardMapper.sumOrderRevenue(todayStart, now)
                + dashboardMapper.sumDonationAmount(todayStart, now);
        long revenueYesterday = dashboardMapper.sumOrderRevenue(yesterdayStart, todayStart)
                + dashboardMapper.sumDonationAmount(yesterdayStart, todayStart);

        long newSubsToday = dashboardMapper.countNewSubscriptions(todayStart, now);
        long newSubsYesterday = dashboardMapper.countNewSubscriptions(yesterdayStart, todayStart);

        long openOrders = dashboardMapper.countOpenOrders();
        long lowStock = dashboardMapper.countLowStock();
        long activeSubscribers = dashboardMapper.countActiveSubscribers();
        ...
        response.setTodayRevenue(KpiDelta.of(revenueToday, revenueYesterday, sevenDayRevenueSpark(now)));
    ...
    public TimeseriesResponse getTimeseries(int days) {
        ...
        List<TrendRow> rows = dashboardMapper.dailyTrend(from, to);
     ...
     */
    public List<FeedItem> getFeed(int limit) {
        int size = limit <= 0 ? DEFAULT_FEED_LIMIT : limit;
        List<FeedRow> rows = dashboardMapper.recentActivity(size);
    ...
    private List<Long> sevenDayRevenueSpark(LocalDateTime now) {
        LocalDate today = now.toLocalDate();
        List<Long> spark = new ArrayList<>(7);
        for (int i = 6; i >= 0; i--) {
            LocalDateTime dayStart = today.minusDays(i).atStartOfDay();
            LocalDateTime dayEnd = dayStart.plusDays(1);
            spark.add(dashboardMapper.sumOrderRevenue(dayStart, dayEnd)
                    + dashboardMapper.sumDonationAmount(dayStart, dayEnd));
        }
        return spark;
    }
