...
public class WorshipAdminController {
    ...
    public ResultDTO<ResInfinityList<WorshipRegistrationListItem>> list(
        ...
        return ResultDTO.ok(worshipService.list(request));
    }

    @Operation(summary = "신청 상세", description = "신청 정보 + 가족 행.")
    @GetMapping("/{id}")
    public ResultDTO<WorshipRegistrationDetail> detail(@PathVariable Long id) {
        return ResultDTO.ok(worshipService.getDetail(id));
    }
    ...
    public ResultDTO<WorshipRegistrationDetail> changeStatus(
        ...
        return ResultDTO.ok(worshipService.changeStatus(id, request));
    }
}
