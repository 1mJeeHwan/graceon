...
public class DonationController {
    ...
    @PostMapping("/list")
    public ResultDTO<ResInfinityList<DonationListItem>> list(@RequestBody DonationSearchRequest request) {
        return ResultDTO.ok(donationService.list(request));
    }

    @Operation(summary = "단건 후원 등록", description = "수기 단건 후원 등록(테스트 모드).")
    @PostMapping("/once")
    public ResultDTO<DonationListItem> createOnce(@Valid @RequestBody OnceDonationRequest request) {
        return ResultDTO.ok(donationService.createOnce(request));
    }

    @Operation(summary = "결제일정 캘린더", description = "활성 구독의 월별 청구 예정 집계(날짜별 건수·금액).")
    @PostMapping("/calendar")
    public ResultDTO<List<BillingCalendarItem>> calendar(@RequestBody BillingCalendarRequest request) {
        return ResultDTO.ok(donationService.billingCalendar(request));
    }
